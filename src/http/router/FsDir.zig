pub const FsDir = @This();

fn fs_dir_handler(ctx: *const http.Context, dir: fs.Dir) !http.Respond {
    ...
    const mime: http.Mime = blk: {
        if (extension_start) |start| {
            if (search_path.len - start == 0) break :blk .BIN;
            break :blk .from_extension(search_path[start + 1 ..]);
        } else {
            break :blk .BIN;
        }
    };
    ...
    hash.update(mem.asBytes(&stat.size));
    if (stat.modified) |modified| {
        hash.update(mem.asBytes(&modified.nanoseconds));
    }
...
pub fn serve(comptime url_path: []const u8, dir: fs.Dir) Middelware.Layer {
    const url_with_match_all = comptime std.fmt.comptimePrint(
        "{s}/%r",
        .{mem.trimEnd(u8, url_path, "/")},
    );
    log.debug("url with match {s}", .{url_with_match_all});

    return Route.init(url_with_match_all).get(
        dir,
        fs_dir_handler,
    ).layer();
}
...
const http = zzz.http;
const fs = tardy.fs;
const Middelware = @import("Middleware.zig");
